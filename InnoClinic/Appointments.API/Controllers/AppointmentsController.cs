public class AppointmentsController : ApiController
{
    ...
    public async Task<IActionResult> GetAppointmentSchedule(int id, DateTime appointmentDate)
    {
        ...
        return result.Match(
            schedule => Ok(_mapper.Map<List<AppointmentResponse>>(schedule)),
        ...
        int? officeId)
    {
        ...
        return result.Match(
            list => Ok(_mapper.Map<List<AppointmentResponse>>(list)),
            errors => Problem(errors));
    }
    ...
    public async Task<IActionResult> GetAppointmentHistory(int id)
    {
        ...
        return result.Match(
            history => Ok(_mapper.Map<List<AppointmentResponse>>(history)),
            errors => Problem(errors));
    }
    ...
    public async Task<IActionResult> RescheduleAppointment(int id, int doctorId, DateTime appointmentDate, TimeSpan appointmentTime)
    {
        ...
        var result = await _mediator.Send(command);
        return result.Match(
            rescheduledAppointment => Ok(_mapper.Map<AppointmentResponse>(rescheduledAppointment)),
            errors => Problem(errors));
    }
