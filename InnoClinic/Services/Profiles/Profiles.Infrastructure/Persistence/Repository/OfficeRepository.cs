...
public class OfficeRepository : IOfficeRepository
{
    ...
    {
        var newOffice = await _dbContext.Office.AddAsync(office);
        return newOffice.Entity;
    }
    
    public async Task DeleteOfficeAsync(string id, CancellationToken cancellationToken = default)
    {
        var office = await _dbContext.Office.FirstOrDefaultAsync(d => d.Id == id);
        if (office != null)
        {
    ...
    {
        return await _dbContext.Office.ToListAsync();
    }

    public async Task<Office> GetOfficeByIdAsync(string id, CancellationToken cancellationToken = default)
    {
        return await _dbContext.Office.FirstOrDefaultAsync(o => o.Id == id);
    }
